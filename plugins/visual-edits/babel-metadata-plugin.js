...
// Tags where <span> children would be invalid HTML
const TABLE_CONTEXT_TAGS = new Set([
  "table", "thead", "tbody", "tfoot", "tr", "colgroup",
  "select", "optgroup", "ol", "ul", "dl", "html", "head",
]);

const wrapDynamicExpressionChildren = (jsxPath, t) => {
  // Skip wrapping if parent element doesn't allow <span> children
  const openingEl = jsxPath.node.openingElement;
  if (openingEl) {
    const tagName = openingEl.name && openingEl.name.name;
    if (tagName && TABLE_CONTEXT_TAGS.has(tagName.toLowerCase())) {
      return; // Don't wrap — <span> is invalid here
    }
  }
